...
var TypeScriptParser = require('jsdoc/src/TypeScriptParser');
...
var Doclet = exports.Doclet = function(docletSrc, meta) {
    ...
    // console.log(this.comment, docletSrc);
...
Doclet.prototype.postFunction = function() {
    ...
    if (info.returnType.string && (!returns || !returns.length || !returns[0].type || !returns[0].type.names.length)) {
        ...
        returns[0].type.names.push(info.returnType.typeId());
    }
    if (!length) {
        return;
    }
    ...
    for (var i = 0; i < length; i++) {
        ...
        if ((!params[i].type || !params[i].type.names.length) && type) {
            params[i].type = {names: [type.typeId()]};
        }
    }
};
...
Doclet.prototype.postTSFunction = function() {
    ...
    function check(info) {
        ...
        if (!isAlreadyReturns) {
            if (!self.returns.length) {
                if (info.returnType.string) {
                    isAdd = true;
                    self.returns.push({
                        type: {names: [info.returnType.typeId()]},
                        description: ''
                    });
                }
                ...
                if (returnnames.indexOf(info.returnType.typeId()) === -1) {
                    if (returnnames.length === 1) {
                        returnnames.push('$ts:...');
                    } else if (returnnames.length < 2) {
                        returnnames.push(info.returnType.typeId());
                        isAdd = true;
                    }
                }
            }
        ...
        info.parameters.forEach(function(param, i) {
            ...
            var type = param.typeAnnotation ? param.typeAnnotation.typeId() : '';
        ...
        if (isAdd &&
            ...
            ((self.typeParameters && self.typeParameters.childs && self.typeParameters.childs.length) || 0) < info.typeParameters.childs.length
        ) {
        ...
        if (!self.extras.filter(function(extra) { return extra.string === info.string; }).length) {
            self.extras.push({
                ...
                parameters: info.parameters.map(function(param) {
                    return {
                        name: param.id,
                        type: {names: param.typeAnnotation.string ? [param.typeAnnotation.typeId()] : []},
                        description: ''
                    };
                })
            });
...
Doclet.prototype.postTypedef = function() {
    ...
    if (interfaceInfo) {
        if (interfaceInfo.typeParameters.string) {
            this.typeParameters = interfaceInfo.typeParameters.string;
        }
        if (!this.type) {
        ...
        interfaceInfo.body.forEach(function(child, i) {
            ...
            self.properties[i] = ({
                ...
                type: {names: [TypeScriptParser.getTypeId(child.typeAnnotation.string)]},
                description: (prevProperty && prevProperty.description) || ''
            });
        });
    ...
    if (typealias) {
        if (!this.type) {
            this.type = {names: [typealias.typeAnnotation.typeId() || 'TSType']};
        }
    }
};
...
Doclet.prototype.postClass = function() {
    ...
    if (classInfo.superClass.string && (!this.augments || !this.augments.length)) {
        this.augments = [classInfo.superClass.typeId()];
    }
};
