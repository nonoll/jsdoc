...
function removeTSParenthesizedType(info) {
    if (info.nodeType === 'TSParenthesizedType') {
        return removeTSParenthesizedType(info.child);
    }

    return info;
}

Parser.prototype.parseExtra = function(filename, info) {
    ...
    if (this._extras[filename][name].every(function(tsInfo) {
        return tsInfo.string !== ts.string;
    })) {
        this._extras[filename][name].push(ts);
    }

    info.extras = this._extras[filename][name];
};
...
Parser.prototype.astnodeToMemberof = function(node) {
    ...
    if ( (type === Syntax.FunctionDeclaration || type === Syntax.FunctionExpression ||
        ...
        var punc = node.static ? jsdoc.name.SCOPE.PUNC.STATIC : jsdoc.name.SCOPE.PUNC.INSTANCE;

        doclet = this._getDocletById(node.enclosingScope.nodeId);

        if (!doclet) {
            result.memberof = jsdoc.name.LONGNAMES.ANONYMOUS + punc;
        }
        else {
            result.memberof = doclet.longname + punc;
        }
    }
