...
exports.getInfo = function(node) {
    ...
    switch (node.type) {
        ...
        case Syntax.ClassDeclaration:
            ...
            info.type = info.node.type;
            info.paramnames = [];
            info.extra = TypeScriptParser.parse(info.node);
        ...
        case Syntax.ClassPrivateProperty:
            ...
            info.type = info.node.type;
            info.extra = TypeScriptParser.parse(info.nod);
            break;
        ...
        case Syntax.ExportNamedDeclaration:
            ...
            if (info.node.declaration) {
                if ( isFunction(info.node.declaration) ) {
                    ...
                    info.extra = TypeScriptParser.parse(info.node.declaration);
                }
                ...
                if (node.declaration.declarations && node.declaration.declarations[0]) {
                    var declarationId = node.declaration.declarations[0].id;

                    if (declarationId.typeAnnotation) {
                        info.extra = TypeScriptParser.parse(declarationId.typeAnnotation);
                    }
                }
        ...
        case 'TSDeclareFunction':
            info.node = node;
            info.name = node.id ? nodeToValue(node.id) : '';
            info.extra = TypeScriptParser.parse(node);
        ...
        case Syntax.VariableDeclarator:
            ...
            // console.log(info.name, TypeScriptParser.parse(node.id).string);
            if (info.name) {
                info.extra = TypeScriptParser.parse(node.id);
            }
            if (node.init) {
                info.type = info.node.type;
                info.value = nodeToValue(info.node);
