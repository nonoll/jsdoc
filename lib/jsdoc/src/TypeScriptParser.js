...
function calcValue(str, info) {
  return (str || "").split("+").map(function (v) {
    ...
    if (str2.indexOf("'") > -1 || str2.indexOf('"') > -1) {
      ...
      return getValue(info[str2], "string", []);
    }
  }).join("");
}
  ...
  return str.replace(/\$([a-z]*)\{((?:(?:'[^']*')|(?:"[^"]*")|(?:[^}]))*)\}/g, function (all, syntax, value) {
    ...
    if (syntax === "if" || syntax === "eq") {
      ...
      if (syntax === "if") {
        ...
        var left = calcValue(values[0], info);
        var right = calcValue(values[1], info);
       
        rv = left === right ? values[2] : values[3];
      }
      
      return calcValue(rv, info);
    }
...
function replaceTemplate(str, info) {
  ...
  info.html = _replaceTemplate(str.replace(/\&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;"), info, "html");
}
...
function debug(func, selector) {
  ...
  if (selector && selector !== "*") {
    if (typeof selector === "string") {
      searchFunc = function (info, node, parentNode, property) {
...
var types = {
  _parse: function(node, parentNode, property) {
    ...
    if (!rv.html) {
      rv.html = replaceHTML(nodeType, rv.string);
    }
    return rv;
  },
  ...
  TSExpressionWithTypeArguments: function TSExpressionWithTypeArguments(node) {
    var info = read(node, {
      ...
      string: "${expression}${typeParameters}",
    });
  ...
  TSTypeParameter: function (node, parentNode) {
    ...
    return read(node, {
      ...
      template: "${name}$if{constraint, ' in ' + constraint}$if{default, ' = ' + default}",
    });
  },
  ...
  TSUndefinedKeyword: function (node) {
    return {
      string: "undefined",
      key: "undefined",
    }
