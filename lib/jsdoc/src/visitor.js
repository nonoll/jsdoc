...
function makeConstructorFinisher(parser) {
    return function(e) {
        ...
        // The priority is ClassDeclaration above Constructor.
...
Visitor.prototype.getVisitComment = function(filename, comment) {
    return this._visitComments[filename] && this._visitComments[filename][comment.start];
};
// TODO: docs
Visitor.prototype.visitNodeComments = function(node, parser, filename) {
    ...
    for (var i = 0, l = comments.length; i < l; i++) {
        ...
        if ( isValidJsdoc(rawComment) ) {
            ...
            e.doclet = this.getVisitComment(filename, comment);
...
Visitor.prototype.visitNode = function(node, parser, filename) {
    ...
    // save doclet with leadingComments
    if (e && e.doclet && node.leadingComments) {
        var leadingComments = node.leadingComments.filter(isBlockComment);

        if (leadingComments.length) {
            // treat the comment closest to the node as the leading comment
            var comment = leadingComments[leadingComments.length - 1];

            if ( !isValidJsdoc(getRawComment(comment)) ) {
                comment = null;
            }
        }
        if (comment) {
            this._visitComments[filename] = this._visitComments[filename] || {};
            this._visitComments[filename][comment.start] = e.doclet;
        }
    }
...
Visitor.prototype.makeSymbolFoundEvent = function(node, parser, filename) {
    ...
    if (extras.code.extra) {
        parser.parseExtra((node.enclosingScope && node.enclosingScope.nodeId) || filename, extras.code);
    }
    if (node.type === 'ImportDeclaration') {
        jsdoc.util.helper.registerImport(filename, TypeScriptParser.parse(node));
    }
    ...
    switch (node.type) {
        ...
        case Syntax.ExportDefaultDeclaration:
            // falls through
            TypeScriptParser.parse(node).specifiers.forEach(function(specifier) {
                jsdoc.util.helper.registerExport(filename, 'default', specifier.key);
            });
