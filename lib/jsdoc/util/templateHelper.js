...
exports.registerExport = function(filename, type, longtype) {
    ...
    if (!exports[filename][type]) {
        exports[filename][type] = longtype;
    }
};
...
function traceType(filename, type) {
    var imports = typeMap.imports[filename];
    var longtype = type;
    var exports = typeMap.exports;

    if (imports && imports[type]) {
        var key = imports[type].key;
        var source = imports[type].source;

        for (var i in exports) {
            if (i.indexOf(source) === 0 && exports[i][key]) {
                longtype = exports[i][key];
                if (key === 'default') {
                    longtype = exports[i][longtype];
                }
            }
        }
    } else if (exports[filename] && exports[filename][type]) {
        longtype = exports[filename][type];
    }
...
function buildLink(longname, linkText, options) {
    ...
    if (longname.indexOf('$ts:') === 0) {
        ...
        return htmlsafe(longname.replace('$ts:', '')).replace(/(("[^"]*")|(&*[a-zA-Z0-9]{2,};*))/g, function(type) {
            if (type === '&amp;' || type === '&lt;' || type[0] === '"' || type[0] === "'") {
                return type;
            }
            var longtype = traceType(filename, type);
            ...
            if (buildLinkMap && buildLinkMap[longtype]) {
                ...
                return '<a href="' + buildLinkMap[longtype] + '"' + target + '>' + type + '</a>';
            } else {
                return type;
            }
        });
...
exports.getMembers = function(data) {
    var members = {
        classes: find( data, {kind: 'class'} ).sort(function(a, b) { return a.sort <= b.sort ? -1 : 1; }),
        ...
        modules: find( data, {kind: 'module'} ).sort(function(a, b) { return a.sort <= b.sort ? -1 : 1; }),
        namespaces: find( data, {kind: 'namespace'} ).sort(function(a, b) { return a.sort <= b.sort ? -1 : 1; }),
        interfaces: find( data, {kind: 'interface'} ).sort(function(a, b) { return a.sort <= b.sort ? -1 : 1; })
    };
